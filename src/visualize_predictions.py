...
def generate_future_taiwan_exports(taiwan_df, end_date='2026-12-01'):
    """Generate future Taiwan exports by extrapolating from recent trends."""
    taiwan_df = taiwan_df.sort_values('date').reset_index(drop=True)
    last_date = taiwan_df['date'].max()
    end_date = pd.Timestamp(end_date)
    
    if last_date >= end_date:
        return taiwan_df
    
    # Use last 12 months to calculate trend
    recent = taiwan_df.tail(12).copy()
    
    # Calculate average and trend
    avg_exports = recent['monthly_exports_usd_millions'].mean()
    # Simple linear trend from last 12 months
    x = np.arange(len(recent))
    y = recent['monthly_exports_usd_millions'].values
    ...
    for i, date in enumerate(future_dates):
        ...
        future_value = trend_coef[0] * (len(recent) + i) + trend_coef[1]
        # Don't let it go negative or too low
        future_value = max(future_value, avg_exports * 0.5)
        
        future_data.append({
            'date': date,
            'monthly_exports_usd_thousands': future_value * 1000,
            'monthly_exports_usd_millions': future_value,
            'year': date.year,
            'month': date.month
        })
    
    future_df = pd.DataFrame(future_data)
    combined_df = pd.concat([taiwan_df, future_df], ignore_index=True)
    return combined_df

def create_features_for_prediction(df):
    """Create features for prediction (same as in predict_nvidia_revenue.py)."""
    df = df.copy()
...
def generate_future_predictions(taiwan_df, end_date='2026-12-01'):
    """Generate Nvidia revenue predictions through end_date using trained model."""
    import joblib
    ...
    if not model_path.exists():
        print("   ⚠️  Trained model not found. Run predict_nvidia_revenue.py first.")
        return None
    ...
    # Generate future Taiwan exports
    taiwan_with_future = generate_future_taiwan_exports(taiwan_df, end_date)
    
    # Create features
    taiwan_with_features = create_features_for_prediction(taiwan_with_future)
    
    # Get only future dates (beyond last actual Taiwan data)
    last_actual_date = taiwan_df['date'].max()
    future_data = taiwan_with_features[taiwan_with_features['date'] > last_actual_date].copy()
    
    if len(future_data) == 0:
        return None
    
    # Prepare features
    X_future = future_data[available_features].fillna(0)
    ...
    future_predictions_df = pd.DataFrame({
        ...
        'taiwan_exports_millions': future_data['monthly_exports_usd_millions'].values,
